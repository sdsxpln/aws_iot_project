...
TMP006DrvOpen(I2C_Handle i2cHandle)
{
	...
	IOT_INFO("Manufacturer ID: 0x%x\n\r", usManufacID);
    if(usManufacID != TMP006_MANUFAC_ID)
    {
        IOT_ERROR("Error in Manufacturer ID\n\r");
        return FAILURE;
    }
	...
	IOT_INFO("Device ID: 0x%x\n\r", usDevID);
    if(usDevID != TMP006_DEVICE_ID)
    {
        IOT_ERROR("Error in Device ID\n");
        return FAILURE;
    }
	...
	IOT_INFO("Configuration register value: 0x%x\n\r", usConfigReg);
